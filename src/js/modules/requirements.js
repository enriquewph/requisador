...
/* global allRequirements:writable, domElements, showToast, updatePreview, reqCounter:writable */
...
function addRequirement() {
  ...
  if (
    !domElements.behaviorInput?.value ||
    !domElements.justificationInput?.value ||
    !domElements.latencyInput?.value
  ) {
...
function moveRequirementUp(index) {
  if (index > 0) {
    [allRequirements[index], allRequirements[index - 1]] = [
      allRequirements[index - 1],
...
function moveRequirementToBottom(index) {
  if (index < allRequirements.length - 1) {
    ...
    console.log(
      `Moved requirement from position ${index} to bottom (position ${allRequirements.length - 1})`
    );
  }
}
...
function convertToLevel2(index) {
  ...
  if (children.length > 0) {
    if (
      !confirm(
        `Este requisito tiene ${children.length} sub-requisito(s). Si lo conviertes a nivel 2, sus hijos se convertirán en requisitos de nivel 1. ¿Continuar?`
      )
    ) {
      return;
    }
...
function findInsertionIndex(parentId) {
  ...
  while (
    insertIndex < allRequirements.length &&
...
function updateParentRequirementOptions() {
  ...
  domElements.parentRequirementSelect.innerHTML =
    '<option value="">Sin requisito padre (Nivel 1)</option>';
...
function buildTreeNode(requirement, children, level) {
  ...
  const behaviorDisplay =
    requirement.behavior.length > 50
...
function showRequirementTooltip(event, requirementId) {
  ...
  const behaviorTruncated =
    requirement.behavior.length > 80
      ? requirement.behavior.substring(0, 80) + '...'
      : requirement.behavior;

  const justificationTruncated =
    requirement.justification.length > 80
